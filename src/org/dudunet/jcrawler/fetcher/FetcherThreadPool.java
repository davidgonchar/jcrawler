 ...
 * @author dudu
 */
public class FetcherThreadPool implements Fetcher {
                ...
                while (true) {
                    try {
                        ...
                        if (response == null) {
                            ...
                            HandlerUtils.sendMessage(handler, new Message(FetcherThreadPool.FETCH_FAILED, page), true);
                            continue;
                        }
...
//                        LogUtils.getLogger().info("fetch " + url);
                        ...
                        if (needUpdateDb) {
                            try {
                                dbUpdater.getSegmentWriter().writeFetch(crawldata);
                                if (isContentStored) {
                                    ...
                                    dbUpdater.getSegmentWriter().writeContent(content);
                                }
                                if (parsing && page.getParseResult() != null) {
                                    dbUpdater.getSegmentWriter().writeParse(page.getParseResult());
                                }
                        ...
                        HandlerUtils.sendMessage(handler, new Message(FetcherThreadPool.FETCH_SUCCESS, page), true);
                    } catch (Exception ex) {
                        LogUtils.getLogger().info("Exception", ex);
                    }
                }
...
//        Busy wait
        do {
            try {
                ...
                LogUtils.getLogger().info("wait interrupted");
            }
